...
According to the official Ethereum documentation \cite{OfficialEthereumLanguages} only four languages for EVM exist: Solidity, Vyper, Yul and Fe. However, other attempts to create language for EVM are known \cite{CommunityEthereumLanguages}. Although there are many EVM languages, Solidity is the most used one, as stated by Harz and Knottenbelt \cite{SolidityWidelyUsed}. Moreover, \cite{SolidityVyperGithubUsage} reports that there are 800 times more Solidity files on Github than Vyper files. It means that majority of existing smart contracts on EVM are written in Solidity. On the one hand it's good because the majority of the community is developing one language on the other it's bad because if a vulnerability occurs in Solidity it occurs on almost every smart contract. Also, community members complains about Solidity language \cite{SolidityDevsChallenges}
